...
/// Projection of an Interface to a complete Signal Rust module.
...
impl InterfaceDocumentReading for InterfaceState {
    ...
    fn finish(self) -> Result<Interface, InterfaceFault> {
        ...
        Ok(Interface {
            ...
            channel: self.channel.ok_or(InterfaceFault::Binding)?,
...
impl InterfaceDocumentWriting for Interface {
    fn textualize_document(&self, scope: &mut TextualizeScope<'_>) -> Result<(), InterfaceFault> {
        ...
        self.channel.write_channel(scope)?;
...
trait OperationElement {
    fn operation(&self) -> &str;
    fn payload(&self) -> &str;
}

impl OperationElement for InputElement {
    fn operation(&self) -> &str {
        &self.operation
    }
    fn payload(&self) -> &str {
...
impl OperationElement for OutputElement {
    fn operation(&self) -> &str {
        &self.operation
    }
    fn payload(&self) -> &str {
...
impl<T: OperationElement> OperationDatomWriting for [T] {
    fn write_operation_datom(
        &self,
        output: &mut String,
        nested_structs: &BTreeSet<String>,
        ...
        output.push_str("            _ => Err(::datom::DatomFault { problem: ::datom::DatomProblem::Shape }),\n        }\n    }\n}\n\nimpl ::datom::DatomTextualizing for ");
        output.push_str(type_name);
        output.push_str(" {\n    fn textualize_in(&self, scope: &mut TextualizeScope<'_>) -> Result<(), ::datom::DatomFault> {\n        match self {\n");
        for input in self {
            if let Some((variant, payload)) = single_record_data_enums.get(input.payload()) {
...
impl SignalArtifactProjecting for Interface {
    fn signal_rust_source(&self) -> Result<String, InterfaceFault> {
        ...
        output.push_str("    }\n}\n\n");
        self.inputs.0.as_slice().write_operation_datom(
            &mut output,
            &nested_structs,
            &unit_enums,
        ...
        self.outputs.0.as_slice().write_operation_datom(
            &mut output,
            &nested_structs,
            &unit_enums,
            &single_record_data_enums,
